#!/usr/bin/env python3
"""High-rate robot_state + RGB-D logging with camera-time alignment.

Captures every /robot_state message at the native publish rate, then aligns
those samples to RGB-D frame timestamps (from camera 0) after each trial.
Output includes:
...
Mocap/OptiTrack support has been removed for the gantry rig (no mocap volume
in this setup). See git history / the stationary-rig version of this file if
that needs to come back for a different experiment.
"""
...
try:
    from .gantry_controller import GantryController, GantryPosition
except ImportError:  # pragma: no cover - supports direct script execution
    from gantry_controller import GantryController, GantryPosition
...
# Gantry defaults. See gantry_controller.py's module docstring for the
# software-tracked-position caveat before trusting the logged X/Y/Z.
GANTRY_PORT = os.environ.get("GANTRY_PORT", "/dev/ttyACM0")
GANTRY_BAUD = 115200
GANTRY_FEEDRATE_MM_MIN = 1200.0
...
def save_session_metadata(
    ...
    if gantry_position is not None:
        payload["gantry_position"] = gantry_position
...
def build_metadata(
    ...
    if gantry_position is not None:
        metadata["gantry_position"] = gantry_position
    return metadata
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument(
        ...
        type=str,
        default=GANTRY_PORT,
        help="Serial port for the gantry's SKR Pro/Marlin controller.",
    )
    ap.add_argument(
        "--gantry-x",
        type=float,
        default=None,
        help=(
            "Target gantry X position in mm for this run, in the software-tracked "
            "frame (0 = wherever the gantry was at connect time). Omit to leave "
            "the gantry wherever it currently is."
        ),
    )
    ap.add_argument(
        "--gantry-y",
        type=float,
        default=None,
        help="Target gantry Y position in mm for this run. See --gantry-x.",
    )
    ap.add_argument(
        "--gantry-z",
        type=float,
        default=None,
        help="Target gantry Z position in mm for this run. See --gantry-x.",
    )
    ap.add_argument(
        "--gantry-feedrate",
        type=float,
        default=GANTRY_FEEDRATE_MM_MIN,
        help="Gantry move feedrate in mm/min.",
    )
    ap.add_argument(
        "--skip-gantry",
        action="store_true",
        help="Don't connect to the gantry at all (e.g. bench-testing cameras/telemetry without it attached).",
    )
...
def main() -> int:
    ...
    if not args.skip_gantry:
        gantry = GantryController(
            ...
            baud=GANTRY_BAUD,
            feedrate=args.gantry_feedrate,
        )
        gantry.connect()
        if args.gantry_x is not None or args.gantry_y is not None or args.gantry_z is not None:
            position = gantry.move_to(x=args.gantry_x, y=args.gantry_y, z=args.gantry_z)
        else:
    ...
    for trial_idx in range(args.trials):
        ...
        metadata = build_metadata(
            ...
            gantry_position=gantry_position_metadata,
        )
        payload = {
